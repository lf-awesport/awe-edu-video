...
const Scene04 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} fadeIn={false} fadeOut={false}><div style={{position: 'absolute', left: 90, top: 68, opacity: head, transform: `translateY(${(1-head)*24}px)`}}><Eyebrow color="#FFC757">Un percorso strutturato</Eyebrow><Title style={{fontSize: 58, marginTop: 15}}>Esplora il <span style={{color: "#FFC757"}}>business dello sport</span></Title></div>
    <Arrow name="arrow-01" draw={stroke(frame, fps, 3.6)} from="top" style={{left: 92, top: 286, width: 132, transform: "rotate(52deg)"}} />
...
const Scene06 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} fadeIn={false} fadeOut={false}><div style={{position: 'absolute', left: 94, top: 80}}><Eyebrow color={brand.colors.cyan}>Gamification · concept UI</Eyebrow><Title style={{marginTop: 22}}>Impara. Avanza. <span style={{color: brand.colors.cyan}}>Sfida.</span></Title></div>
...
const Scene10 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene}><div style={{position:'absolute',left:92,top:90}}><Eyebrow color="#EF8621">Una roadmap condivisa</Eyebrow><Title style={{marginTop:24}}>Obiettivi e modalità<br/><span style={{color:'#EF8621'}}>costruiti insieme.</span></Title></div><div style={{position:'absolute',left:300,right:300,top:470,height:18,borderRadius:12,background:'#D6DFEA'}}><div style={{height:'100%',width:`${p*100}%`,borderRadius:12,background:'linear-gradient(90deg,#EF8621,#FFC757)'}}/>{[0,.25,.5,.75,1].map((v,i)=><div key={v} style={{position:'absolute',left:`${v*100}%`,top:-18,width:54,height:54,borderRadius:'50%',background:p>=v?'#EF8621':'#fff',border:'6px solid #fff',boxShadow:'0 5px 20px rgba(0,30,80,.2)',transform:'translateX(-50%)'}}><div style={{position:'absolute',top:70,left:'50%',transform:'translateX(-50%)',width:170,textAlign:'center',fontSize:19,fontWeight:800}}>STEP {i+1}</div></div>)}</div><Arrow name="arrow-07" draw={stroke(frame, fps, 3)} from="left" style={{left: 1540, top: 606, width: 190, transform: "rotate(-9deg)"}} /><div style={{position:'absolute',left:300,width:1320,top:700,textAlign:'center',fontFamily:brand.fonts.body,fontSize:28,color:brand.colors.white}}>Durata e milestone definite con il partner.</div><div style={{position:'absolute',right:120,top:110,padding:'18px 24px',borderRadius:16,background:brand.colors.blue,color:brand.colors.white,fontWeight:900,letterSpacing:1.4}}>PROGETTO SU MISURA</div></SceneShell>;
};
...
const Scene11 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene}><div style={{position:'absolute',left:92,top:75}}><Eyebrow color="#A2CD4B">Attiva la community</Eyebrow><Title style={{fontSize:63,marginTop:20}}>Connetti le persone.<br/><span style={{color:'#A2CD4B'}}>Premia il percorso.</span></Title></div><div style={{position:'absolute',left:0,top:0,right:0,bottom:0}}>{(() => {
