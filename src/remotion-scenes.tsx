...
// Pieces of the standard that compose into an element's own transform rather
// than replacing it: the blur that clears as it arrives, the travel that
// overshoots its resting place, and the four-frame step between siblings.
const openBlur = (progress: number) => `blur(${((1 - progress) * MOTION.blur).toFixed(2)}px)`;
const settle = (progress: number, distance: number) => interpolate(progress, [0, .72, 1], [distance, -distance * MOTION.overshoot, 0]);
const step = (fps: number, base: number, index: number) => base + (index * MOTION.staggerFrames) / fps;
...
// as a mask over a white sheet and grown until it covers the frame.
const LogoWipe = ({progress}: {progress: number}) => {
  const size = interpolate(progress, [0, 1], [170, 6400], {...clamp, easing: Easing.inOut(Easing.cubic)});
  const source = `url("${brandAsset('brand/logos/awe-sport-education-mark-white.svg')}")`;
  const box = `${size.toFixed(1)}px ${size.toFixed(1)}px`;
  return <AbsoluteFill style={{background: brand.colors.white, WebkitMaskImage: source, maskImage: source, WebkitMaskSize: box, maskSize: box, WebkitMaskPosition: 'center', maskPosition: 'center', WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat'}} />;
};
...
const SceneShell = ({scene, children, fadeIn = false, fadeOut = false, surface = 'magma'}: SceneProps & {children: ReactNode; fadeIn?: boolean; fadeOut?: boolean; surface?: 'magma' | 'white'}) => {
  ...
  const light = surface === 'white';
  return <AbsoluteFill style={{background: light ? brand.colors.white : brand.colors.ink, color: light ? brand.colors.ink : brand.colors.white, fontFamily: brand.fonts.display, overflow: 'hidden', opacity}}>
    {!light && <Img src={brandAsset('brand/backgrounds/runtime/magma-gradient.png')} style={{position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover'}} />}
    {children}
  </AbsoluteFill>;
};
...
const Scene02 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} fadeIn={false} fadeOut={false}>
    <div style={{position:'absolute',inset:0,display:'grid',placeItems:'center'}}><div style={{opacity:logoIn*(1-out),filter:openBlur(logoIn),transform:`translateY(${settle(logoIn,40)-drift*14-out*90}px) scale(${.92+logoIn*.08+drift*.02})`,transformOrigin:'center'}}><Logo width={560}/></div></div>
  </SceneShell>;
};
...
const Scene04 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} fadeIn={false} fadeOut={false}>
    ...
    <div style={{position: 'absolute', left: 226, right: 226, top: 250, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, perspective: 1400}}>{subjects.map((subject, i) => {
      ...
      const lead = (i * MOTION.staggerFrames) / fps;
      const arrival = interpolate(frame / fps - lead, [.2, 1.7], [0, 1], {...clamp, easing: Easing.out(Easing.cubic)});
      const emphasis = Math.max(0, 1 - Math.abs(focus - i));
      const x = settle(arrival, 210 + column * 46) + out * (90 + column * 55);
      const y = settle(arrival, 170 + row * 44) - out * (40 + row * 26);
      const depth = settle(arrival, 430) - out * 260;
      const rotation = settle(arrival, (i - 5.5) * 1.2) + out * (i - 5.5) * .8;
      return <div key={subject} style={{height: 200, borderRadius: 18, overflow: 'hidden', position: 'relative', boxShadow: `0 ${10+emphasis*18}px ${28+emphasis*32}px rgba(11,42,91,${.16+emphasis*.2})`, opacity: arrival * (.78 + emphasis * .22) * (1 - out), filter: openBlur(arrival), zIndex: 20-Math.abs(i-5.5), outline:`${emphasis*6}px solid ${subjectColors[i]}`, transform: `translate3d(${x}px,${y-emphasis*18}px,${depth}px) rotateZ(${rotation}deg) scale(${1+emphasis*.075})`, transformOrigin: 'center'}}><Img src={brandAsset(`subjects/runtime/${subject}.png`)} style={{width: '100%', height: '100%', objectFit: 'cover',filter:`saturate(${.82+emphasis*.38}) brightness(${.9+emphasis*.1})`}} /><div style={{position: 'absolute', left: 13, top: 13, width: 10, height: 35, borderRadius: 8, background: subjectColors[i]}} /></div>})}</div>
</SceneShell>;
};
...
const Scene05 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene}>
    ...
    <div style={{position: 'absolute', left: 1010, top: 158, width: 760, height: 1360, borderRadius: 62, padding: 14, background: '#071E4F', boxShadow: '0 44px 120px rgba(0,20,80,.45)', opacity: phoneIn, filter: openBlur(phoneIn), transform: `translate(${out * 320}px, ${settle(phoneIn, 90)}px) rotate(${-3 + enter * 1.4 + out * 4}deg) scale(${1 + enter * .03})`, transformOrigin: '40% 28%'}}>
...
const Scene09 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} surface="white"><Copy frame={frame} fps={fps} out={out} style={{left:92,top:72}}>
    ...
    </Copy><div style={{position:'absolute',left:92,right:92,top:340,display:'flex',gap:28,transform:`translateY(${out*240}px)`,opacity:1-out*.4}}>{courses.map(([asset,title],i)=>{const r=reveal(frame,fps,step(fps,.5,i));const active=Math.max(0,1-Math.abs(focus-i));return <div key={asset} style={{width:550,height:470,borderRadius:26,overflow:'hidden',background:'#fff',color:brand.colors.ink,opacity:r,filter:openBlur(r),border:'1px solid rgba(10,36,88,.12)',boxShadow:`0 ${14+active*22}px ${34+active*60}px rgba(10,36,88,.16)`,transform:`translateY(${settle(r,60)-active*22}px) scale(${1+active*.035})`}}><Img src={brandAsset(`subjects/runtime/${asset}.png`)} style={{width:'100%',height:310,objectFit:'cover',transform:`scale(${1+active*.06})`}}/><div style={{padding:'24px 28px'}}><div style={{fontSize:16,fontWeight:800,color:brand.colors.blue,letterSpacing:2}}>CASE STUDY · ESEMPIO</div><div style={{fontSize:29,fontWeight:900,marginTop:10}}>{title}</div></div></div>})}</div></SceneShell>;
};
...
const Scene11 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} surface="white"><Copy frame={frame} fps={fps} out={out} style={{left:92,top:75}}>
      ...
      return <>
        ...
        {groups.map((g,i)=>{const angle=i*Math.PI/2+.35+orbit*.42;const r=reveal(frame,fps,step(fps,.6,i));return <div key={g} style={{position:'absolute',left:centre.x+Math.cos(angle)*radius.x,top:centre.y+Math.sin(angle)*radius.y,width:node.width,height:node.height,borderRadius:18,background:'#fff',border:'1px solid rgba(10,36,88,.14)',boxShadow:'0 10px 26px rgba(10,36,88,.10)',color:brand.colors.ink,display:'grid',placeItems:'center',fontSize:18,fontWeight:900,opacity:r,filter:openBlur(r),transform:`translate(-50%,-50%) scale(${r})`}}>{g}</div>})}
      </>;
    })()}</div><div style={{position:'absolute',right:95,top:350,width:760,opacity:1-out,transform:`translateX(${out*280}px)`}}>{rewards.map((reward,i)=>{const r=reveal(frame,fps,step(fps,1,i));const active=Math.max(0,1-Math.abs(rewardFocus-i));return <div key={reward} style={{height:125,marginBottom:18,borderRadius:24,background:i===0?'#A2CD4B':'#F1F5FA',color:brand.colors.ink,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 36px',opacity:r,filter:openBlur(r),outline:`${active*3}px solid #A2CD4B`,transform:`translateX(${settle(r,65)-active*18}px) scale(${1+active*.025})`}}><span style={{fontSize:27,fontWeight:900}}>{reward}</span><span style={{fontFamily:brand.fonts.body,fontSize:19}}>Esempio da validare →</span></div>})}</div></SceneShell>;
};
...
const Scene12 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} surface="white"><Copy frame={frame} fps={fps} out={out} style={{left:92,top:72}}>
    ...
    </Copy><div style={{position:'absolute',left:92,top:350,width:1050,height:540,borderRadius:28,overflow:'hidden',background:'#122D61',border:'1px solid #ffffff20',opacity:r*(1-out),filter:openBlur(r),transform:`translateX(${settle(r,-60)-out*280}px)`}}><Img src={brandAsset('subjects/runtime/media.png')} style={{width:'100%',height:'100%',objectFit:'cover',opacity:.75,transform:`scale(${zoom})`}}/><div style={{position:'absolute',inset:0,background:'linear-gradient(transparent 45%,rgba(3,12,38,.86))'}}/><div style={{position:'absolute',left:28,top:26,padding:'12px 18px',borderRadius:12,background:'#EC264F',fontWeight:900,letterSpacing:2}}>INCONTRO · DEMO</div><div style={{position:'absolute',left:35,bottom:32,fontSize:30,fontWeight:900}}>Sport Industry Q&amp;A</div></div><div style={{position:'absolute',right:92,top:350,width:610,height:540,borderRadius:28,background:'#fff',border:'1px solid rgba(10,36,88,.12)',boxShadow:'0 16px 40px rgba(10,36,88,.12)',color:brand.colors.ink,padding:32,opacity:reveal(frame,fps,.8)*(1-out),transform:`translateX(${out*300}px)`}}><div style={{fontSize:22,fontWeight:900,color:brand.colors.blue}}>DOMANDE · UI DIMOSTRATIVA</div>{messages.map((m,i)=>{const messageIn=reveal(frame,fps,delays[i]);return <div key={m} style={{marginTop:28,padding:'20px 22px',borderRadius:18,background:i===2?'#E6F8FD':'#EEF2F8',fontFamily:brand.fonts.body,fontSize:21,opacity:messageIn,filter:openBlur(messageIn),transform:`translateX(${settle(messageIn,38)}px)`}}>{m}</div>})}</div></SceneShell>;
};

const Scene13 = ({scene}: SceneProps) => {
  const frame = useCurrentFrame(); const {fps} = useVideoConfig();
  const t = frame / fps;
  // the white of the previous scenes closes back into the mark, which shrinks
  // into place before the wordmark joins it
  const LOCKUP = 860, MARK_SHARE = 179 / 623;
  const markSize = interpolate(t, [0, 1.25], [6400, LOCKUP * MARK_SHARE], {...clamp, easing: Easing.inOut(Easing.cubic)});
  const markCentreX = interpolate(t, [1.35, 2.05], [960, 960 - LOCKUP / 2 + (LOCKUP * MARK_SHARE) / 2], {...clamp, easing: Easing.inOut(Easing.cubic)});
  const handover = interpolate(t, [2.0, 2.45], [0, 1], {...clamp, easing: Easing.inOut(Easing.cubic)});
  const cta = reveal(frame, fps, 3.1);
  const source = `url("${brandAsset('brand/logos/awe-sport-education-mark-white.svg')}")`;
  const box = `${markSize.toFixed(1)}px ${markSize.toFixed(1)}px`;
  const spot = `${(markCentreX - markSize / 2).toFixed(1)}px ${(540 - markSize / 2).toFixed(1)}px`;
  return <SceneShell scene={scene}>
    <AbsoluteFill style={{background: brand.colors.white, opacity: 1 - handover, WebkitMaskImage: source, maskImage: source, WebkitMaskSize: box, maskSize: box, WebkitMaskPosition: spot, maskPosition: spot, WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat'}} />
    <div style={{position: 'absolute', inset: 0, display: 'grid', placeItems: 'center'}}>
      <div style={{opacity: handover}}><Logo width={LOCKUP} /></div>
    </div>
    <div style={{position: 'absolute', left: 0, right: 0, top: 700, textAlign: 'center', fontSize: 66, lineHeight: 1.04, fontWeight: 900, letterSpacing: -3, opacity: cta, filter: openBlur(cta), transform: `translateY(${settle(cta, 42)}px)`}}>Porta la formazione sportiva<br /><span style={{color: brand.colors.white}}>nella tua community.</span></div>
  </SceneShell>;
};
