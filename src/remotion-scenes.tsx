...
const SceneShell = ({scene, children, accent = brand.colors.cyan, fadeIn = false, fadeOut = false}: SceneProps & {children: ReactNode; accent?: string; fadeIn?: boolean; fadeOut?: boolean}) => {
...
const Scene05 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} accent="#43A7DE"><div style={{position: 'absolute', left: 95, top: 80, width: 630, opacity: left}}><Eyebrow color="#43A7DE">Un’esperienza fluida</Eyebrow><Title style={{marginTop: 24}}>Impara.<br /><span style={{color: '#43A7DE'}}>Passo dopo passo.</span></Title><div style={{marginTop: 30, fontFamily: brand.fonts.body, fontSize: 30, lineHeight: 1.3}}>Contenuti e lezioni progettati per accompagnare l’utente su ogni dispositivo.</div></div>
...
const Scene11 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} accent="#A2CD4B"><div style={{position:'absolute',left:92,top:75}}><Eyebrow color="#A2CD4B">Attiva la community</Eyebrow><Title style={{fontSize:63,marginTop:20}}>Connetti le persone.<br/><span style={{color:'#A2CD4B'}}>Premia il percorso.</span></Title></div><div style={{position:'absolute',left:0,top:0,right:0,bottom:0}}>{(() => {
