...
const Scene04 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} light accent="#FFC757"><div style={{position: 'absolute', left: 90, top: 68, opacity: head}}><Eyebrow color={brand.colors.blue}>Un percorso strutturato</Eyebrow><Title style={{fontSize: 58, marginTop: 15}}>12 prospettive sullo <span style={{color: brand.colors.blue}}>sport business</span></Title></div>
    ...
    <div style={{position: 'absolute', right: 88, top: 175, padding: '11px 18px', borderRadius: 12, background: '#FFF0D6', color: '#A04325', fontSize: 16, fontWeight: 800}}>Numeri e classificazione da approvare</div></SceneShell>;
};
...
const Scene06 = ({scene}: SceneProps) => {
  ...
  const ranks = [['01','Alex R.','2.480'],['02','Marta B.','2.160'],['03','Sam K.','1.940']];
  return <SceneShell scene={scene} accent="#A2CD4B"><div style={{position: 'absolute', left: 94, top: 80}}><Eyebrow color="#A2CD4B">Gamification · concept UI</Eyebrow><Title style={{marginTop: 22}}>Impara. Avanza. <span style={{color: '#A2CD4B'}}>Sfida.</span></Title></div>
    ...
    <div style={{position: 'absolute', left: 720, right: 95, top: 310, height: 535, borderRadius: 30, background: '#082358', padding: 42, opacity: reveal(frame,fps,.8)}}><div style={{fontSize: 23, fontWeight: 800, color: '#A2CD4B'}}>CLASSIFICA · ESEMPIO</div>{ranks.map((rank,i)=><div key={rank[0]} style={{marginTop: 24, height: 112, borderRadius: 18, display: 'grid', gridTemplateColumns: '90px 1fr 150px', alignItems: 'center', padding: '0 28px', background: i===0?'#A2CD4B':'#ffffff10', color: i===0?brand.colors.ink:'#fff', opacity: reveal(frame,fps,1.2+i*.25)}}><span style={{fontSize: 30, fontWeight: 900}}>{rank[0]}</span><span style={{fontSize: 28, fontWeight: 800}}>{rank[1]}</span><span style={{fontSize: 25, fontWeight: 900, textAlign: 'right'}}>{rank[2]} XP</span></div>)}</div>
  </SceneShell>;
};

const Scene07 = ({scene}: SceneProps) => {
  const frame = useCurrentFrame(); const {fps} = useVideoConfig(); const paper = reveal(frame,fps,.25); const seal = reveal(frame,fps,1.5);
  return <SceneShell scene={scene} accent="#FFC757"><div style={{position:'absolute',left:92,top:92,width:610}}><Eyebrow color="#FFC757">Il traguardo</Eyebrow><Title style={{marginTop:22}}>Le competenze<br /><span style={{color:'#FFC757'}}>che hai costruito.</span></Title><div style={{fontFamily:brand.fonts.body,fontSize:28,lineHeight:1.35,marginTop:30}}>Il certificato finale verrà inserito solo dopo la validazione del template ufficiale.</div></div>
    <div style={{position:'absolute',right:120,top:120,width:850,height:750,background:'#FFFCF4',color:brand.colors.ink,borderRadius:12,padding:'70px 78px',boxShadow:'0 40px 90px rgba(0,0,30,.38)',opacity:paper,transform:`translateY(${(1-paper)*90}px) rotate(2deg)`}}><Logo color width={245}/><div style={{marginTop:80,fontFamily:brand.fonts.body,fontSize:19,letterSpacing:4,color:'#73809A'}}>CERTIFICATO DI COMPLETAMENTO</div><div style={{marginTop:32,fontSize:48,fontWeight:900}}>Nome partecipante</div><div style={{height:2,background:'#CAD4E5',marginTop:22}}/><div style={{marginTop:38,fontFamily:brand.fonts.body,fontSize:24,lineHeight:1.45}}>ha completato il percorso formativo<br/><strong>AWE Sport Education</strong></div><div style={{position:'absolute',right:70,bottom:60,width:150,height:150,borderRadius:'50%',border:`10px double ${brand.colors.blue}`,display:'grid',placeItems:'center',fontWeight:900,fontSize:19,textAlign:'center',opacity:seal,transform:`scale(${seal})`}}>TEMPLATE<br/>DA VALIDARE</div></div>
  </SceneShell>;
};
...
const Scene10 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} light accent="#EF8621"><div style={{position:'absolute',left:92,top:90}}><Eyebrow color="#EF8621">Una roadmap chiara</Eyebrow><Title style={{marginTop:24}}>Dall’onboarding<br/><span style={{color:'#EF8621'}}>al traguardo.</span></Title></div><div style={{position:'absolute',left:720,right:120,top:430,height:18,borderRadius:12,background:'#D6DFEA'}}><div style={{height:'100%',width:`${p*100}%`,borderRadius:12,background:'linear-gradient(90deg,#EF8621,#FFC757)'}}/>{[0,.25,.5,.75,1].map((v,i)=><div key={v} style={{position:'absolute',left:`${v*100}%`,top:-18,width:54,height:54,borderRadius:'50%',background:p>=v?'#EF8621':'#fff',border:'6px solid #fff',boxShadow:'0 5px 20px rgba(0,30,80,.2)',transform:'translateX(-50%)'}}><div style={{position:'absolute',top:70,left:'50%',transform:'translateX(-50%)',width:170,textAlign:'center',fontSize:19,fontWeight:800}}>STEP {i+1}</div></div>)}</div><div style={{position:'absolute',left:745,top:660,fontFamily:brand.fonts.body,fontSize:28,color:'#52637E'}}>Durata e milestone definitive da approvare.</div><div style={{position:'absolute',right:120,top:110,padding:'18px 24px',borderRadius:16,background:'#FFF0D6',color:'#A04325',fontWeight:900}}>FINO A 2 MESI · CLAIM NON VERIFICATO</div></SceneShell>;
};

const Scene11 = ({scene}: SceneProps) => {
  const frame=useCurrentFrame(); const {fps}=useVideoConfig(); const groups=['FAN','COMMUNITY','STUDENTI','STAKEHOLDER']; const rewards=['MERCH','ESPERIENZE','STAGE'];
...
const Scene12 = ({scene}: SceneProps) => {
  ...
  return <SceneShell scene={scene} accent="#EC264F"><div style={{position:'absolute',left:92,top:72}}><Eyebrow color="#FF7592">Sessioni live · concept</Eyebrow><Title style={{fontSize:62,marginTop:18}}>Incontra. Condividi.<br/><span style={{color:'#FF7592'}}>Interagisci.</span></Title></div><div style={{position:'absolute',left:92,top:350,width:1050,height:540,borderRadius:28,overflow:'hidden',background:'#122D61',border:'1px solid #ffffff20',opacity:r}}><Img src={brandAsset('subjects/runtime/media.png')} style={{width:'100%',height:'100%',objectFit:'cover',opacity:.75}}/><div style={{position:'absolute',inset:0,background:'linear-gradient(transparent 45%,rgba(3,12,38,.86))'}}/><div style={{position:'absolute',left:28,top:26,padding:'12px 18px',borderRadius:12,background:'#EC264F',fontWeight:900,letterSpacing:2}}>● LIVE · DEMO</div><div style={{position:'absolute',left:35,bottom:32,fontSize:30,fontWeight:900}}>Sport Industry Q&amp;A</div></div><div style={{position:'absolute',right:92,top:350,width:610,height:540,borderRadius:28,background:'#fff',color:brand.colors.ink,padding:32,opacity:reveal(frame,fps,.8)}}><div style={{fontSize:22,fontWeight:900,color:brand.colors.blue}}>CHAT · UI DIMOSTRATIVA</div>{messages.map((m,i)=><div key={m} style={{marginTop:28,padding:'20px 22px',borderRadius:18,background:i===2?'#E6F8FD':'#EEF2F8',fontFamily:brand.fonts.body,fontSize:21,opacity:reveal(frame,fps,1.2+i*.35)}}>{m}</div>)}</div></SceneShell>;
};
